...
export default function SkillCard({ skill, selected, onSelect, showCheckbox }: Props) {
      ...
      {showCheckbox && (
        <div
          className={`absolute top-3 right-3 w-5 h-5 rounded border-2 flex items-center justify-center transition-colors
            ${selected ? 'bg-sky-500 border-sky-500 text-white' : 'border-gray-600 group-hover:border-sky-500'}`}>
  ...
  if (showCheckbox) {
    return (
      <div
        ...
        className={`relative group bg-gray-900 border rounded-xl p-5 transition-all duration-200 cursor-pointer
          hover:border-sky-500/50 select-none
          ${selected ? 'border-sky-500 ring-1 ring-sky-500/50 bg-sky-500/5' : 'border-white/5'}`}>
        {cardContent}
      </div>
    )
  }
  ...
  return (
    <Link
      ...
      className={`relative group bg-gray-900 border rounded-xl p-5 transition-all duration-200 block
